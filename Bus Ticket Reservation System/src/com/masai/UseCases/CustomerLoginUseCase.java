...
public class CustomerLoginUseCase {
    ...
    public static List<CustomerDTO> avalibleTicket() {
        ...
        try (Connection conn = DBUtil.provideConnection()) {
            PreparedStatement ps = conn.prepareStatement("select bus_no, custer_id,sum(noof_ticket) from Ticket_book  GROUP BY  bus_No");
            ...
            while (rs.next()) {
                ...
                PreparedStatement ps1 = conn.prepareStatement("select seat from bus where  busNo=?");
